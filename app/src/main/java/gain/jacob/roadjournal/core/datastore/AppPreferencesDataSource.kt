package gain.jacob.roadjournal.core.datastore
...
private val Context.dataStore by preferencesDataStore("roadjournal_settings")
...
class AppPreferencesDataSource @Inject constructor(@ApplicationContext context: Context) {
    ...
    val settings: Flow<AppSettings> = store.data.map { values ->
        AppSettings(
            ...
            themeMode = values[THEME_MODE].enumOr(gain.jacob.roadjournal.core.model.ThemeMode.SYSTEM),
            accentTheme = values[ACCENT_THEME].enumOr(gain.jacob.roadjournal.core.model.AccentTheme.DEFAULT),
            useDynamicColor = values[DYNAMIC_COLOR] ?: true,
            useAmoledBlack = values[AMOLED] ?: false,
            displayDensity = values[DENSITY].enumOr(gain.jacob.roadjournal.core.model.DisplayDensity.COMFORTABLE),
    ...
    suspend fun updateAppearance(
        themeMode: gain.jacob.roadjournal.core.model.ThemeMode? = null,
        accentTheme: gain.jacob.roadjournal.core.model.AccentTheme? = null,
        dynamicColor: Boolean? = null,
        amoled: Boolean? = null,
        density: gain.jacob.roadjournal.core.model.DisplayDensity? = null,
    ) { store.edit { values ->
