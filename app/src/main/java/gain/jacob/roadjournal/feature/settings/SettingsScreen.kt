package gain.jacob.roadjournal.feature.settings
...
import gain.jacob.roadjournal.core.util.displayDate
...
import gain.jacob.roadjournal.BuildConfig
...
@Composable fun SettingsScreen(onMessage:(String)->Unit,viewModel:SettingsViewModel=hiltViewModel()){
    ...
    LazyColumn(Modifier.fillMaxSize(),contentPadding=PaddingValues(20.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
        ...
        item{Section("Data")}
        item{ListItem(headlineContent={Text("Export backup")},supportingContent={Text("Save all vehicles and readings as JSON")},modifier=Modifier.clickable{json.launch("roadjournal-backup.json")})}
        item{ListItem(headlineContent={Text("Restore backup")},supportingContent={Text("Replace local data from RoadJournal JSON")},modifier=Modifier.clickable{open.launch(arrayOf("application/json","text/plain"))})}
        ...
        item{Section("About")};item{ListItem(headlineContent={Text("RoadJournal")},supportingContent={Text("Version ${BuildConfig.VERSION_NAME} · Your data stays on this device unless you export it.")})}
