...
import com.jacobgain.triprabbit.core.designsystem.component.OdometerDisplay
...
fun DashboardScreen(onAdd:(Long)->Unit,onHistory:(Long)->Unit,onManageVehicles:()->Unit,onStatistics:(Long)->Unit,viewModel:DashboardViewModel=hiltViewModel()) {
    ...
    if(vehicle==null){com.jacobgain.triprabbit.core.designsystem.component.EmptyState("No vehicles","Add a vehicle to begin.","Add Vehicle",onManageVehicles);return}
    val latest=state.readings.firstOrNull();val previous=state.readings.getOrNull(1)
    LazyColumn(Modifier.fillMaxSize(),contentPadding=PaddingValues(20.dp),verticalArrangement=Arrangement.spacedBy(18.dp)){
        item{Text("TripRabbit",style=MaterialTheme.typography.headlineMedium)}
