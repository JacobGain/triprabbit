...
class EndToEndWorkflowTest {
    ...
    @Before fun launchFreshApp(){
        ...
        context.deleteDatabase("roadlog.db")
        context.filesDir.resolve("datastore/app.preferences_pb").delete()
        context.startActivity(Intent(context,Class.forName("com.roadlog.MainActivity")).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK))
        waitFor("Get Started")
    }
    ...
    @Test fun createVehicleAddReadingAndEditHistory(){
        ...
        waitFor("RoadLog");waitForContains("Golf");waitFor("120,000")
        ...
        device.pressBack();waitFor("RoadLog");click("Vehicles");clickDescription("Add Vehicle");waitFor("Create Vehicle")
        setEditField(0,"Civic",2);setEditField(1,"80000",2);click("Create Vehicle")
        waitFor("RoadLog");waitForContains("Civic");waitFor("80,000")
        click("Add Reading");setEditField(0,"80100",1);device.pressBack();click("Save Reading");waitFor("Reading added");waitFor("RoadLog");waitFor("80,100")
        ...
        device.pressHome();val context=ApplicationProvider.getApplicationContext<Context>();context.startActivity(Intent(context,Class.forName("com.roadlog.MainActivity")).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK));waitFor("RoadLog");waitForContains("Golf");waitFor("120,400")
